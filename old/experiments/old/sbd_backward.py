...
class SBDB(optim.Optimizer):
    def __init__(self, params, model, obj, eta=None, n=1, momentum=0, projection_fn=None, weight_decay=0, eps=1e-8, adjusted_momentum=False, betas=(0.0, 0.99), amsgrad=False):
        ...
        super(SBDB, self).__init__(params_list, defaults)
        ...
        self.N = 1 # max number of lower bounds
        self.n = 0 # number of lower bounds
        self.max_grads = self.N # max number of none zero lower bounds
    ...
    def step(self, loss, x, y):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                self.state[p]['w_s'].append(p.data.detach().clone())
        self.n += 1

        if len(self.losses) > self.max_grads:
            self.losses = self.losses[1:]
            self.n = self.N
            for group in self.param_groups:
                for p in group['params']:
                    self.state[p]['grads'] = self.state[p]['grads'][1:]
                    self.state[p]['w_s'] = self.state[p]['w_s'][1:]
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                for i in range(loop_range):
                    ...
                    w_s = self.state[p]['w_s']
                    self.b[i] -= g_i.mul(w_s[i]-w_s[-1]).sum()
