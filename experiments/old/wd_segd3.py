...
class SEGD3(optim.Optimizer):
    ...
    @torch.autograd.no_grad()
    def compute_extra_gradient(self, x, y):
        ...
        for group in self.param_groups:
            for p in group['params']:
                self.state[p]['g_e'] = (p.grad.data + group['weight_decay'] * p.data).detach().clone()
                self.loss_w_e += 0.5 * group['weight_decay'] * p.data.norm()**2

    @torch.autograd.no_grad()
    def compute_gradient(self, closure):

        self.loss_w_t = closure()
        for group in self.param_groups:
            for p in group['params']:
                self.state[p]['w_t'] = p.data.detach().clone()
                self.state[p]['g_t'] = (p.grad.data + group['weight_decay'] * p.data).detach().clone()
                self.loss_w_t += 0.5 * group['weight_decay'] * p.data.norm()**2


    @torch.autograd.no_grad()
    def compute_step_size(self):
        # calculate step size
        loss_t = float(self.loss_w_t)
        loss_e = float(self.loss_w_e)
    ...
    def step(self, loss, x, y):
        ...
        self.compute_extra_gradient(x, y)
